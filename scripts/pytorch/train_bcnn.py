...
def train(data_path, batch_size, max_epoch, pretrained_model,
    ...
    for epo in range(max_epoch):
        ...
        for index, (in_feature, out_feature_gt) in enumerate(train_dataloader):
            ...
            category_loss, confidence_loss, class_loss, instance_x_loss, instance_y_loss, heading_loss, height_loss\
                = criterion(output, in_feature, out_feature_gt, category_weight, confidence_weight, class_weight)
            loss = category_loss + confidence_loss + class_loss + instance_x_loss + instance_y_loss + heading_loss + height_loss
            ...
            loss_for_record = category_loss + confidence_loss + \
                              class_loss + instance_x_loss + instance_y_loss + heading_loss + height_loss
        ...
        vis.line(X=np.array([epo]), Y=np.array([avg_heading_train_loss]), win='loss',
                 name='avg_heading_train_loss', update='append')
