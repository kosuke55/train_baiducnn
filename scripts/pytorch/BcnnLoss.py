...
class BcnnLoss(Module):
    ...
    def forward(self, input, output, target, weight, class_weight):
        gamma = 3.0
        alpha = 1.0
        ...
        #     output[:, 4:10, ...] + 1e-7)) * (1.0 + torch.log(alpha * input[:, 2:3, ...]))) * 0.01
        # class_loss = -torch.sum((1.0 - output[:, 4:10, ...]) ** gamma * (target[:, 4:10, ...] * torch.log(
        #     output[:, 4:10, ...] + 1e-7))) * 0.01
        class_loss \
        ...
        instance_loss = torch.sum(
            (weight * instance_x_diff) ** 2 + (weight * instance_y_diff) ** 2) * 0.001

        height_diff = output[:, 11, ...] - target[:, 11, ...]
        height_loss = torch.sum((weight * height_diff) ** 2) * 0.001
