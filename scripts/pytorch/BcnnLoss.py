...
class BcnnLoss(Module):
    ...
    def forward(self, output, input, target, category_weight, confidence_weight, class_weight):
        ...
        category_loss = torch.sum(category_weight * object_weight * torch.where(category_diff <= sigma, 0.5 * (
            category_diff**2), sigma * torch.abs(category_diff) - 0.5 * (sigma ** 2)) * (1.0 + beta * input[:, 2:3, ...])) * 0.005
        ...
        confidence_loss = torch.sum(confidence_weight * object_weight*torch.where(confidence_diff <= sigma, 0.5 * (
            confidence_diff**2), sigma * torch.abs(confidence_diff) - 0.5 * (sigma ** 2))) * 0.005
        ...
        class_loss = -torch.sum(class_weight *
                                (((1.0 - output[:, 4:9, ...]) ** gamma) * (target[:, 4:9, ...] * torch.log(output[:, 4:9, ...] + 1e-7)) * alpha +
                                 ((output[:, 4:9, ...]) ** gamma) * ((1.0 - target[:, 4:9, ...]) * torch.log(1.0 - output[:, 4:9, ...] + 1e-7)) * (1.0 - alpha))
                                * (1.0 + beta * input[:, 2:3, ...])) * 0.01

        instance_x_diff = output[:, 1, ...] - target[:, 1, ...]
        instance_y_diff = output[:, 2, ...] - target[:, 2, ...]
        instance_x_loss = torch.sum(
            torch.where(instance_x_diff <= sigma, 0.5 * (instance_x_diff**2), sigma * torch.abs(instance_x_diff) - 0.5 * (sigma ** 2)) * (1.0 + beta * input[:, 2:3, ...])) * 0.005
        instance_y_loss = torch.sum(
            torch.where(instance_y_diff <= sigma, 0.5 * (instance_y_diff**2), sigma * torch.abs(instance_y_diff) - 0.5 * (sigma ** 2)) * (1.0 + beta * input[:, 2:3, ...])) * 0.005
        # instance_x_loss = torch.sum(
        ...
        heading_x_loss = torch.sum(
            torch.where(heading_x_diff <= sigma, 0.5 * (heading_x_diff**2), sigma * torch.abs(heading_x_diff) - 0.5 * (sigma ** 2)) * (1.0 + beta * input[:, 2:3, ...])) * 0.005
        heading_y_loss = torch.sum(
            torch.where(heading_y_diff <= sigma, 0.5 * (heading_y_diff**2), sigma * torch.abs(heading_y_diff) - 0.5 * (sigma ** 2)) * (1.0 + beta * input[:, 2:3, ...])) * 0.005
        ...
        height_loss = torch.sum(torch.where(height_diff <= sigma, 0.5 * (height_diff**2), sigma * torch.abs(
            height_diff) - 0.5 * (sigma ** 2)) * (1.0 + beta * input[:, 2:3, ...])) * 0.00025
