...
infh = h5py.File('100_nusc_baidu_confidence.h5', 'r')
infh.keys()
...
cv2.imwrite("image/loss_weight.png", loss_weight)
...
cv2.imwrite("image/confidence_out.png", out_image)
...
if(vis_all_in_feature):
    for i in range(8):
        ...
        cv2.imwrite("image/confidence_in_{}.png".format(i), in_image)
else:
    in_image = in_feature.reshape(640, 640, 8)
    in_image = in_image[:, :, 0]
    in_image[np.where(in_image != 0)] = 255
    in_image = in_image.astype(np.uint8)
    cv2.imwrite("image/confidence_in_{}.png".format(0), in_image)
